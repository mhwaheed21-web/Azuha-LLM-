...
#-----------------------------------------------------------------------------------------------------------
#above code is used to generate response of model using old fomat
#below is the code that using new format and is used to generate response of model using user/azuha chat format
#--------------------------------------------------------------------------------------------------------------
...
# finetune_generate.py
# Generate responses from Azuha — User/Azuha chat format
#
# Run after finetune_train.py has saved the model.
# Usage: python fine_tunning_step5/finetune_generate.py
...
# ─────────────────────────────────────────────
# MODEL CONFIG — must match finetune_train.py exactly
# ─────────────────────────────────────────────
...
CHOOSE_MODEL = "gpt2-small (124M)"
LORA_RANK    = 4   # must match what was used in finetune_train.py
...
def load_finetuned_model(model_path, device):
    """
    Loads the fine-tuned Azuha model.
    apply_lora() must be called BEFORE load_state_dict()
    because the saved file contains LoRA weights (lora.A and lora.B).
    Without applying LoRA first the keys will not match and it will crash.
    """
    cfg   = {**BASE_CONFIG, **MODEL_CONFIGS[CHOOSE_MODEL]}
    model = GPTModel(cfg)

    # inject LoRA structure first so model matches saved file
    apply_lora(model, rank=LORA_RANK)
    ...
    model.to(device)
    model.eval()
    print(f"Loaded Azuha from {model_path}")
    return model
...
def generate_response(model, tokenizer, device,
                      user_message, extra_input="",
                      max_new_tokens=150,
                      temperature=0.0,
                      top_k=50):
    """
    Takes a plain user message, formats it as:
        User: <user_message>
        Azuha:
    Then generates the response after 'Azuha:' and returns it.

    Args:
        user_message  : what the user typed
        extra_input   : optional context (usually empty)
        max_new_tokens: max tokens to generate
        temperature   : 0.0 = deterministic greedy
                        0.7 = more varied and creative
        top_k         : sample from top k tokens (None = no filter)
    """
...
def evaluate_on_test_set(model, tokenizer, device, test_data, num_samples=10):
    """
    Generates responses for num_samples entries from test_data
    and prints them alongside expected outputs.
    """
    print("\n" + "="*60)
    print("AZUHA — TEST SET EVALUATION")
    print("="*60)
    ...
    for i, item in enumerate(test_data[:num_samples]):
        ...
        print(f"Expected: {item['assistant']}")
...
if __name__ == "__main__":
    ...
    print("\n" + "="*60)
    print("AZUHA — Personal Assistant")
